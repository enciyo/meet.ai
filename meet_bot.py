...
class MeetBot:
    def __init__(self, meet_url, user_name, output_file_name, language="tr-TR", api_key=None):
        ...
        self.speech_recognition = SpeechRecognition(language=language, api_key=api_key, file_path=output_file_name)
        self.summarizer = Summarizer(api_key=api_key, language=language)

    def on_finish(self):
        logging.info("Google Meet is finished")
    ...
    def start(self):
        logging.info(
            f"Starting MeetBot with url: {self.meet_url}, user: {self.user_name}, output file: {self.output_file_name}")
